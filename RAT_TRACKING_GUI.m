...
for ratidx=ratpt
    ratobj{ratidx}.max_speed=min(max(val,1e-3),30);
end
...
val=str2double(get(hObject,'String'));
for ratidx=ratpt
    ratobj{ratidx}.max_floorlength=min(max(val,1e-2),100);
end
...
val=str2double(get(hObject,'String'));
for ratidx=ratpt
    ratobj{ratidx}.mass=min(max(val,1e-2),1000);
end
...
val=str2double(get(hObject,'String'));
for ratidx=ratpt
    ratobj{ratidx}.max_ratlength=min(max(val,0.01),0.5);
end
...
val=str2double(get(hObject,'String'));
for ratidx=ratpt
    ratobj{ratidx}.max_ratwidth=min(max(val,0.001),0.1);
end
...
val=str2double(get(hObject,'String'));
for ratidx=ratpt
    ratobj{ratidx}.hindlimb_length=min(max(val,0.001),0.05);
end

function BUTTON_INIT_VIDEO_Callback(~, ~, handles)
global ratobj ratpt;
ratobj{ratpt(1)}.initial_process(handles.PANEL_RAWVIDEO,1);
